...
void inicializar_habilidades(){
    ...
    init_habilidad(ataquesusto, "Bola Fantasmal", "Los fantasmas desaparacen para asustarte y quitarte vida y puntos de defensa", 10, 0, 5);

    Habilidad *proteccioneterea = malloc(sizeof(Habilidad));
    init_habilidad(proteccioneterea, "Protección Etérea", "El fantasma se vuelve intangible, reduciendo el daño recibido", 0, 10, 0);

    // Cofre trampa
    Habilidad *mordisco = malloc(sizeof(Habilidad));
    init_habilidad(mordisco, "Mordisco", "El cofre se abre y te muerde para romper tus protecciones", 20, 0, 0);

    Habilidad *defensadura = malloc(sizeof(Habilidad));
    init_habilidad(defensadura, "Defensa Dura", "El cofre se cierra herméticamente, aumentando su defensa", 0, 15, 0);

    // Elfo Subordinado
    Habilidad *lanzamientocuchillos = malloc(sizeof(Habilidad));
    init_habilidad(lanzamientocuchillos, "Lanzamiento de Cuchillos", "El elfo subordinado saca un manojo de cuchillos y te los lanza, estos te inmovilizan los brazos y te costara mas atacar", 10, 0, 0);

    Habilidad *esquivarapido = malloc(sizeof(Habilidad));
    init_habilidad(esquivarapido, "Esquiva Rápida", "El elfo subordinado esquiva rápidamente los ataques", 0, 10, 0);

    // Elfo Supremo
    Habilidad *mazazo = malloc(sizeof(Habilidad));
    init_habilidad(mazazo, "Mazazo", "El elfo supremo saca una maza que tenía guardada en la cabeza y te golpea en la cabeza con ella", 10, 0, 20);

    Habilidad *armaduramagica = malloc(sizeof(Habilidad));
    init_habilidad(armaduramagica, "Armadura Mágica", "El elfo supremo invoca una armadura mágica que reduce el daño recibido", 0, 15, 0);

    // Tortuguita
    Habilidad *ataqueacuatico = malloc(sizeof(Habilidad));
    init_habilidad(ataqueacuatico, "Ataque Acuático", "Las tortuguitas se sumergen bajo el agua y saltan a por ti las dos a la vez complicando tu defensa", 20, 00, 0);

    Habilidad *caparazonreforzado = malloc(sizeof(Habilidad));
    init_habilidad(caparazonreforzado, "Caparazón Reforzado", "La tortuguita se esconde bajo su caparazón reforzado, aumentando su defensa", 0, 15, 0);

    // Tortugón
    Habilidad *golpepesado = malloc(sizeof(Habilidad));
    init_habilidad(golpepesado, "Golpe Pesado", "El tortugón lanza un golpe pesado con su caparazón", 25, 0, 0);

    Habilidad *caparazoneterno = malloc(sizeof(Habilidad));
    init_habilidad(caparazoneterno, "Caparazón Eterno", "El tortugón se esconde bajo el caparazón y rechaza parte de tus ataques", 0, 10, 0);

    // Mini Blaze
    Habilidad *llamaradas = malloc(sizeof(Habilidad));
    init_habilidad(llamaradas, "Llamaradas", "Los mini blazes te acorralan con llamaradas de fuego", 20, 0, 0);

    Habilidad *escudoflamigero = malloc(sizeof(Habilidad));
    init_habilidad(escudoflamigero, "Escudo Flamígero", "El mini blaze crea un escudo de llamas que reduce el daño recibido", 0, 20, 0);

    // Blaze
    Habilidad *bloqueatomico = malloc(sizeof(Habilidad));
    init_habilidad(bloqueatomico, "Bloque Atómico", "El blaze te lanza un bloque atómico gigantesco que cuando te golpea explota ", 20, 0, 0);

    Habilidad *pareddefuego = malloc(sizeof(Habilidad));
    init_habilidad(pareddefuego, "Pared de Fuego", "El blaze invoca una pared de fuego que bloquea los ataques entrantes", 0, 20, 0);

    Habilidad *tormentadefuego = malloc(sizeof(Habilidad));
    init_habilidad(tormentadefuego, "Tormenta de fuego", "El Blaze invoca una tormenta de fuego que causa daño a todos los enemigos cercanos", 30, 0, 0);

    Habilidad *escudoinfernal = malloc(sizeof(Habilidad));
    init_habilidad(escudoinfernal, "Escudo infernal", "El Blaze crea una bola de fuego gigante y se mete en ella que reduce el daño recibido y quema a los atacantes", 20, 10, 0);
...
void inicializar_enemigos(Habilidad *habilidades[][4]){
    ...
    init_enemigo(fantasma1, "Fantasma de soldado", 100, 20, 10,habilidades[0],2);

    Enemigo *fantasma2 = malloc(sizeof(Enemigo));
    init_enemigo(fantasma2, "Fantasma de soldado", 100, 20, 10,habilidades[0],2);
    
    Enemigo *cofretrampa = malloc(sizeof(Enemigo));
    init_enemigo(cofretrampa, "Cofre Trampa", 125, 20, 20,habilidades[1],2);

    //Creación de enemigos escenario 2, con sus respectivas habilidades
    Enemigo *subordinadoelfo = malloc(sizeof(Enemigo));
    init_enemigo(subordinadoelfo, "Subordinado elfo", 125, 25, 20,habilidades[2],2); 
    
    Enemigo *jefoelfe = malloc(sizeof(Enemigo));    
    init_enemigo(jefoelfe, "Elfo Jefe", 100, 30, 30,habilidades[3],2);
     
    //Creación de enemigos escenario 3, con sus respectivas habilidades
    Enemigo *tortuguita1=malloc(sizeof(Enemigo));
    init_enemigo(tortuguita1, "Tortugas", 105, 25, 10,habilidades[4],2);

    Enemigo *tortuguita2=malloc(sizeof(Enemigo));;
    init_enemigo(tortuguita2, "Tortugas", 105, 25, 10,habilidades[4],2);

    Enemigo *tortugon = malloc(sizeof(Enemigo));
    init_enemigo(tortugon, "Tortuga", 105, 30, 100,habilidades[5],2);
    
    //Creación de enemigos escenario 4, con sus respectivas habilidades
    Enemigo *miniblaze = malloc(sizeof(Enemigo));
    init_enemigo(miniblaze, "Secuaz de Blaze", 120, 30, 10,habilidades[6],2);
    
    Enemigo *blaze = malloc(sizeof(Enemigo));
    init_enemigo(blaze, "BLAZE", 120, 70, 30,habilidades[7],4);
...
void inicializar_opcion(Enemigo *enemigos[][2]){
    ...
    init_opcion(opcion2_3,"Derecha","Las voces continuan intentando apodersarse de ti, pero no te encuentras a ningun enemigo",NULL,0,"Consigues salir del bosque tenebroso sin pelear con nadie");
    ...
    init_opcion(opcion4_1, "Ventana", "Rompes la ventana y entras al castillo pero te toca a enfrentarte a al BLAZE y a los mini blazes", enemigos[5], 2, "Has derrotado al boss final y recuperas a la princesa. Tu aventura ha llegado a su FIN");

    Opcion *opcion4_2=malloc(sizeof(Opcion)); 
    init_opcion(opcion4_2, "Puerta", "Consigues forzar la puerta  entrar al castillo y librarte de los mini blazes, pero ahora te toca enfrentarte al BLAZE", enemigos[6], 1, "Has derrotado al boss final y recuperas a la princesa. Tu aventura ha llegado a su FIN");

    Opcion *opcion4_3=malloc(sizeof(Opcion)); 
    init_opcion(opcion4_3, "Sotano", "Entrando por el sotano te libras de entrentarte contra los mini blazes, pero aún te queda el BLAZE", enemigos[6], 1, "Has derrotado al boss final y recuperas a la princesa. Tu aventura ha llegado a su FIN ");
...
void inicializar_decision(Opcion *opciones[][3]){
    ...
    init_decision(decision4,"Por donde quieres entrar?", opciones[3], 3);
...
void inicializar_escenario(Decision *decisiones[]){
    ...
    init_escenario(escenario1, "Cementerio de KINGSLAND", 
    ...
    init_escenario(escenario3, "Lago Magistral", 
    ...
    init_escenario(escenario4, "El gran castillo de KINGSLAND", 
    ...
    &decisiones[3],1);
    Escenario *escenarios[4] = {escenario1,escenario2,escenario3,escenario4};
    inicializar_juego(escenarios, 4);
...
void inicializar_juego(Escenario *escenarios[], int num_escenarios){
    ...
    init_habilidad(&habilidadesAtaque[0], "Golpe Fuerte", "Un golpe poderoso que aumenta el ataque.", 50, 0, 0);
    init_habilidad(&habilidadesAtaque[1], "Ataque Rápido", "Un ataque rápido que golpea primero.", 50, 0, 0);
    init_habilidad(&habilidadesAtaque[2], "Explosión de Fuego", "Una explosión que inflige daño a múltiples enemigos.", 50, 0, 0);
    init_habilidad(&habilidadesAtaque[3], "Rayo", "Un rayo que atraviesa la defensa enemiga.", 50, 0, 0);

    Habilidad habilidadesDefensa[4];
    init_habilidad(&habilidadesDefensa[0], "Defensa de Hierro", "Una defensa impenetrable que aumenta la defensa.", 0, 50, 0);
    init_habilidad(&habilidadesDefensa[1], "Escudo Mágico", "Un escudo que reduce el daño recibido.", 0, 50, 0);
    init_habilidad(&habilidadesDefensa[2], "Curación Rápida", "Recupera puntos de vida rápidamente.", 0, 50, 0);
    init_habilidad(&habilidadesDefensa[3], "Resistencia", "Aumenta la resistencia a los ataques enemigos.", 0, 50, 0);

    Personaje* miPersonaje = malloc(sizeof(Personaje));
    init_personaje(miPersonaje, habilidadesAtaque, 4, habilidadesDefensa, 4); 
    printf("Tu personaje es el siguiente:\n");
    print_personaje(miPersonaje);

    Movimiento* historial = NULL;

    Grafo* mi_grafo = malloc(sizeof(Grafo));
    init_grafo(mi_grafo); // Inicializa el grafo

    //Jugamos el primer escenario
    jugar_escenario(miPersonaje,escenarios[mi_grafo->nodo_actual],&historial);

    //Preguntamos y jugamos los demas grafos
    while (mi_grafo->nodo_actual < 3) {
        imprimir_opciones(mi_grafo,escenarios);
        int opcion;
        printf("Selecciona una opción: ");
        scanf("%d", &opcion);
        if(opcion == 0){
            mostrar_historial(mi_grafo,escenarios);
        }else{
            mover(mi_grafo, opcion);
            jugar_escenario(miPersonaje,escenarios[mi_grafo->nodo_actual],&historial);
        }
    }
}
