...
void inicializar_habilidades(){
    //Creación de las habilidades de cada enemmigo del juego
...
void inicializar_opcion(Enemigo *enemigos[][2]){
    ...
    init_opcion(opcion2_1,"Izquierda","Has tenido suerte y no te pierdes en el laberinto, tampoco te encuentras a ningun enemigo ",NULL,0,"Consigues llegar al lago sin pelear con nadie");

    Opcion *opcion2_2=malloc(sizeof(Opcion));  
    init_opcion(opcion2_2, "Medio", "Pasas por varios ostaculos y en medio de uno de ellos te ecuentros a un elfo subordinado y su jefe, te enfrentas a ellos", enemigos[2], 2, "Has derrotado a los elfos y llegas al lago.");

    Opcion *opcion2_3=malloc(sizeof(Opcion));  
    init_opcion(opcion2_2,"Derecha","Las voces continuan intentando apodersarse de ti, pero no te encuentras a ningun enemigo",NULL,0,"Consigues llegar al lago sin pelear con nadie");
    
    //Opciones escenario 3, con los enemigos correspondientes en cada opción
    Opcion *opcion3_1=malloc(sizeof(Opcion));  
    init_opcion(opcion3_1, "Izquierda", "Andaras con cuidado de no caer al agua pero no te encontraras a ningun enemigo en medio", NULL, 0, "Llegas al castillo sin tener que luchar contra ningun enemigo");

    Opcion *opcion3_2=malloc(sizeof(Opcion));  
    init_opcion(opcion3_2, "Medio", "Por este camino tendras que vecer a a las dos tortuguitas armadas que intentaran que no llegues al castillo", enemigos[3], 2, "Después de vencer a las dos torutguitas, finalmente llegas al castillo ");

    Opcion *opcion3_3=malloc(sizeof(Opcion)); 
    init_opcion(opcion3_3,"Derecha","A pesar de que has escogido el camino rápido vas a tener que perder tiempo luchando contra el tortugon ",enemigos[4],1,"Después de vencer al tortugon, finalmente llegas al castillo");
...
void inicializar_escenario(Decision *decisiones[]){
    // Creación de los escenarios, con su decisión correspondiente donde dentro de esta se encuentran las opciones con los enemigos
    
    Escenario *escenario1=malloc(sizeof(Escenario));
    init_escenario(escenario1, "Escenario 1", 
    "Acabas de aparecer en medio del cementerio de KINGSLAND, la ciudad que salvaste, y desconcertado, te encuentras con la gran maga SCARLET, la hermana menor de la princesa. Esta estaba rezando en tu tumba y se ha quedado anonadada de que tú estés delante suya. El gran héroe, después de tantos años, ha revivido. Llorando desconsoladamente, te pide ayuda para salvar a su hermana. Para ello, Scarlet te ha devuelto todos los ataques especiales que tenías en el pasado.\n\n"
    ...
    "EN LA DERECHA ENCUENTRAS EL SÓTANO, EL CUAL RECUERDAS QUE TIENE UNA VÍA DIRECTA HACIA DONDE SE ENCUENTRAN LA PRINCESA Y BLAZE.",
    decisiones[3]);
